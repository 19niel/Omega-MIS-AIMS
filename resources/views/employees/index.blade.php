            ...
            <div class="max-w-7xl mx-auto">
                <div class="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p class="text-sm text-slate-500">Manage system users, tracking assets and employment status.</p>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr class="text-xs font-bold text-slate-400 uppercase tracking-wider">
                                ...
                                <th class="px-6 py-4">Position & Dept</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($employees as $employee)
                                <tr class="hover:bg-slate-50 transition-colors cursor-pointer">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-slate-600 font-medium">{{ $employee->position }}</div>
                                        <div class="text-[10px] text-slate-400 uppercase font-bold tracking-tight">{{ $employee->department }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase 
                                            {{ $employee->status === 'Active' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-slate-100 text-slate-700 border-slate-200' }}">
                                            {{ $employee->status }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-1.5">
                                            <span class="text-sm font-bold text-slate-700">
                                                {{ is_array($employee->current_assets) ? count($employee->current_assets) : 0 }}
                                            </span>
                                            <span class="text-[10px] text-slate-400 uppercase">Items</span>
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-slate-500 text-sm italic">
